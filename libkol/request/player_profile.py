...
class player_profile(Request[bool]):
    ...
    async def parser(content: str, **kwargs) -> bool:
        ...
        session.state.username = username_match.group(1)
        session.state.num_ascensions = (
            int(ascensions_match.group(1)) if ascensions_match else 0
        )
        session.state.num_trophies = (
            int(trophies_match.group(1)) if trophies_match else 0
        )
        session.state.num_tattoos = int(tattoos_match.group(1)) if tattoos_match else 0

        clanMatch = clan.search(content)
        if clanMatch:
            clan_id = int(clanMatch.group(1))
            clan_name = clanMatch.group(2)
            session.state.clan = Clan(session, id=clan_id, name=clan_name)
