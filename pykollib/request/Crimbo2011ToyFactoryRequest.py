...
class Crimbo2011ToyFactoryRequest(GenericRequest):
    ...
    def parseResponse(self):
        ...
        giftAlreadyReceivedPattern = PatternManager.getOrCompilePattern(
            "crimboUserAlreadyReceivedGift"
        )
        if giftAlreadyReceivedPattern.search(self.responseText):
            raise Error.Error(
                "That player has already received that gift.", Error.ALREADY_COMPLETED
            )
