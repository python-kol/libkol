...
class clan_raid_log(Request[Raid]):
    ...
    log_patterns = {
        re.compile(r"^got the carriageman ([0-9]+) sheet\(s\) drunker$"): RaidAction.DreadCarriageman,
        re.compile(r"^used The Machine, assisted by (?P<left_chamber>.*?) and (?P<right_chamber>.*?)$"): RaidAction.DreadMachineUse,
        ...
        re.compile(r"^was defeated by +(?:a )?(?P<monster>.*?) \((?P<turns>1) turn\)$"): RaidAction.Defeat,
        re.compile(r"^was defeated by +(?:a )?(?P<monster>.*?) x [0-9,]+ {}$".format(turns_p)): RaidAction.Defeat,
        re.compile(r"^defeated +(?:a )?(?P<monster>.*?) \((?P<turns>1) turn\)$"): RaidAction.Victory,
        re.compile(r"^defeated +(?:a )?(?P<monster>.*?) x [0-9,]+ {}$".format(turns_p)): RaidAction.Victory,
        re.compile(r"^unlocked the (?P<location>.*?) \(1 turn\)"): RaidAction.DreadUnlock,
        re.compile(r"^fixed The Machine"): RaidAction.DreadMachineFix,
        re.compile(r"^raided a (?P<location>dresser)"): RaidAction.DreadFreddy,
        re.compile(r"^drove some (?P<type>.*?) out of the (?P<location>.*?) \(1 turn\)"): RaidAction.DreadBanishType,
        re.compile(r"^sifted through some (?P<location>ashes)"): RaidAction.DreadFreddy,
        re.compile(r"^(acquired|got) (some|a) (?P<item>.*?) \(1 turn\)"): RaidAction.DreadGotItem,
        re.compile(r"^made a shepherd's pie"): RaidAction.DreadShepherdsPie,
        re.compile(r"^looted the blacksmith's (?P<location>till)"): RaidAction.DreadFreddy,
        ...
        re.compile(r"^made the (?P<location>.*?) less (?P<element>.*?) \(1 turn\)$"): RaidAction.DreadBanishElement,
        re.compile(r"^recycled some (?P<location>newspapers)"): RaidAction.DreadFreddy,
        re.compile(r"^found and sold a rare (?P<location>baseball card)"): RaidAction.DreadFreddy,
        re.compile(r"^made an impression of a complicated lock"): RaidAction.DreadLockImpression,
        re.compile(r"^lowered the water level"): RaidAction.HoboSewerWaterLevel,
        re.compile(r"^gnawed through (?:[0-9]+|a) C. H. U. M. cages? {}$".format(turns_p)): RaidAction.HoboSewerCageGnaw,
        re.compile(r"^made it through the sewer \((?P<turns>[0-9]+) turns?\)$"): RaidAction.HoboSewerClear,
        re.compile(r"^explored (a|[0-9]+) dark sewer tunnels? {}$".format(turns_p)): RaidAction.HoboSewerExplore,
        re.compile(r"^went shopping in the Marketplace {}$".format(turns_p)): RaidAction.HoboSquareMarketShop,
        re.compile(r"^squeezed a Slime gall bladder \(1 turn\)"): RaidAction.SlimeSqueeze,
        re.compile(r"^distributed (?P<item>.*?) to {}$".format(user_p)): RaidAction.Distribute,
        re.compile(r"^rifled through a (?P<location>footlocker)"): RaidAction.DreadFreddy,
        ...
        re.compile(r"^got intimate with some hot coals \(1 turn\)$"): RaidAction.DreadBuffHot,
        re.compile(r"^learned to make a moon-amber necklace \(1 turn\)$"): RaidAction.DreadMoonAmberNecklace,
        re.compile(r"^got magically fingered \(1 turn\)$"): RaidAction.DreadBuffMP,
        re.compile(r"^made (a|some) cool iron (?P<item>.*?) \(1 turn\)$"): RaidAction.DreadCoolIronItem,
        re.compile(r"^read some lurid epitaphs \(1 turn\)$"): RaidAction.DreadBuffSleaze,
        ...
        re.compile(r"^lifted some weights \(1 turn\)$"): RaidAction.DreadStatMuscleForest,
        re.compile(r"^rolled around in some mushrooms \(1 turn\)$"): RaidAction.DreadBuffDefence,
        re.compile(r"^drank some nutritious forest goo \(1 turn\)$"): RaidAction.DreadBuffHP,
        re.compile(r"^made a clockwork bird"): RaidAction.DreadClockworkBird,
        re.compile(r"^stared at (?:an|[0-9]+) empty cages? for (?:a|[0-9]+) whiles? {}$".format(turns_p)): RaidAction.HoboSewerCageStare,
        re.compile(r"^rescued (?P<other_user>.*?) from a C. H. U. M. cage {}$".format(turns_p)): RaidAction.HoboSewerCageFree,
        re.compile(r"^opened (?:a|[0-9]+) sewer grates? {}$".format(turns_p)): RaidAction.HoboSewerGrates,
        re.compile(r"^helped Richard make (?:a|[0-9,]+) (?P<item>protein shake|grenade|bandage)s? {}$".format(turns_p)): RaidAction.HoboRichard,
        re.compile(r"^threw (a|[0-9]+) tires? on the fire {}$".format(turns_p)): RaidAction.HoboHotTireBuild,
        re.compile(r"^started (?:a|[0-9]+) tirevalanches? {}$".format(turns_p)): RaidAction.HoboHotTireCollapse,
        re.compile(r"^got burned by (?:a|[0-9]+) hot doors? {}$".format(turns_p)): RaidAction.HoboHotDoor,
        re.compile(r"^raided (a|[0-9]+) freezers? {}$".format(turns_p)): RaidAction.HoboColdFreezer,
        re.compile(r"^yodeled (?P<amount>a little bit|quite a bit|like crazy) {}$".format(turns_p)): RaidAction.HoboColdYodel,
        re.compile(r"^busted (a|[0-9]+) moves? {}$".format(turns_p)): RaidAction.HoboSpookyDanceSuccess,
        re.compile(r"^watched some zombie hobos dance {}$".format(turns_p)): RaidAction.HoboSpookyWatch,
        re.compile(r"^bamboozled some hobos {}$".format(turns_p)): RaidAction.HoboSleazeBamboozle,
        re.compile(r"^flimflammed some hobos {}$".format(turns_p)): RaidAction.HoboSleazeFlimflam,
        re.compile(r"^danced like a superstar {}$".format(turns_p)): RaidAction.HoboSleazePickNose,
        re.compile(r"^started (a|[0-9]+) barfights? {}$".format(turns_p)): RaidAction.HoboSleazeBarfight,
        re.compile(r"^started (a|[0-9]+) trashcano eruptions? {}$".format(turns_p)): RaidAction.HoboStenchEruption,
        re.compile(r"^searched for buried treasure {}$".format(turns_p)): RaidAction.HoboStenchTreasure,
        re.compile(r"^swam in a sewer \(1 turn\)$"): RaidAction.DreadBuffStench,
        re.compile(r"^listened to the forest's heart \(1 turn\)$"): RaidAction.DreadStatMysticalityForest,
        ...
        re.compile(r"^did a whole bunch of pushups \(1 turn\)$"): RaidAction.DreadStatMuscleCastle,
        re.compile(r"^moved some bricks around \(1 turn\)$"): RaidAction.DreadStatMuscleVillage,
        re.compile(r"^flipped through a photo album \(1 turn\)$"): RaidAction.DreadStatMoxieForest,
        re.compile(r"^diverted some cold water out of Exposure Esplanade {}$".format(turns_p)): RaidAction.HoboColdPipeTurnFirst,
        re.compile(r"^diverted some cold water to Burnbarrel Blvd. {}$".format(turns_p)): RaidAction.HoboColdPipeTurnSecond,
        re.compile(r"^broke (?:a|[0-9]+) water pipes? {}$".format(turns_p)): RaidAction.HoboColdPipeBreak,
        re.compile(r"^wasted some fruit \(1 turn\)$"): RaidAction.DreadKiwiWaste,
        re.compile(r"^\(unknown action\: (?P<action>.*?)\) {}$".format(turns_p)): RaidAction.Unknown,
        re.compile(r"^{}$".format(turns_p)): RaidAction.Unknown,
        re.compile(r"^raided some naughty cabinets \(1 turn\)$"): RaidAction.DreadStatMoxieVillage,
        re.compile(r"^read some naughty carvings \(1 turn\)$"): RaidAction.DreadStatMysticalityVillage,
        re.compile(r"^sent some trash to The Heap {}$".format(turns_p)): RaidAction.HoboSleazyTrash,
        re.compile(r"^sent (?:some|[0-9]+ bunches of) flowers to The Heap {}$".format(turns_p)): RaidAction.HoboSpookyFlowers,
        re.compile(r"^sent (?:a|[0-9]+) batch(?:es)? of compost to the Burial Ground {}$".format(turns_p)): RaidAction.HoboStenchCompost,
        re.compile(r"^found (?:a|[0-9]+) caches? of Meat for the clan coffer {}$".format(turns_p)): RaidAction.HoboHotMeat,
        re.compile(r"^diverted some steam away from Burnbarrel Blvd. {}$".format(turns_p)): RaidAction.HoboHotPipeTurn,
        re.compile(r"^raided (?:a|[0-9]+) dumpsters? {}$".format(turns_p)): RaidAction.HoboSleazeMeat,
        re.compile(r"^raided (?:a|[0-9]+) fridges? {}$".format(turns_p)): RaidAction.HoboColdMeat,
        re.compile(r"^raided (?:a|[0-9]+) tombs? {}$".format(turns_p)): RaidAction.HoboSpookyMeat,
        re.compile(r"^took a nap on a prison cot \(1 turn\)$"): RaidAction.DreadMP,
        re.compile(r"^slew some vampires {}$".format(turns_p)): RaidAction.SororityKillVampires,
        re.compile(r"^took care of some werewolves {}$".format(turns_p)): RaidAction.SororityKillWerewolves,
        re.compile(r"^took out some zombies {}$".format(turns_p)): RaidAction.SororityKillZombies,
        re.compile(r"^took out some skeletons {}$".format(turns_p)): RaidAction.SororityKillSkeletons,
        re.compile(r"^trapped some ghosts {}$".format(turns_p)): RaidAction.SororityKillGhosts,
        re.compile(r"^picked up some staff guides {}$".format(turns_p)): RaidAction.SororityKillGuides,
        re.compile(r"^snagged a ghost trap {}$".format(turns_p)): RaidAction.SororityItemGhost,
        re.compile(r"^snagged a funhouse mirror {}$".format(turns_p)): RaidAction.SororityItemSkeleton,
        re.compile(r"^made a silver shotgun shell {}$".format(turns_p)): RaidAction.SororityItemWerewolf,
        re.compile(r"^grabbed a chainsaw chain {}$".format(turns_p)): RaidAction.SororityItemZombie,
        re.compile(r"^grabbed a sexy costume {}$".format(turns_p)): RaidAction.SororityCostume,
        re.compile(r"^turned (?P<direction>up|down) the fog machine {}$".format(turns_p)): RaidAction.SororityFogMachine,
        re.compile(r"^(?:cranked|turned) (?P<direction>up|down) the spooky noise volume {}$".format(turns_p)): RaidAction.SororityBoombox,
        re.compile(r"^(?P<direction>opened|closed)(?: up)? some windows {}$".format(turns_p)): RaidAction.SororityDownWindows,
        re.compile(r"^passed (?:the|[0-9]+) hats? in the tent {}$".format(turns_p)): RaidAction.HoboSquareBusk,
        re.compile(r"^took the stage {}$".format(turns_p)): RaidAction.HoboSquarePerform,
        re.compile(r"^failed to impress as a dancer {}$".format(turns_p)): RaidAction.HoboSpookyDanceFailure,
        re.compile(r"^started (?:a|[0-9]+) mosh pits? in the tent {}$".format(turns_p)): RaidAction.HoboSquareMosh,
        re.compile(r"^ruined (?:the|[0-9]+) shows? {}$".format(turns_p)): RaidAction.HoboSquareLeave,
        re.compile(r"^had a drink in the Marketplace {}$".format(turns_p)): RaidAction.HoboSquareMarketBooze,
        re.compile(r"^had some food in the Marketplace {}$".format(turns_p)): RaidAction.HoboSquareMarketFood,
        re.compile(r"^read some ancient secrets \(1 turn\)$"): RaidAction.DreadStatMysticalityCastle,
    }
