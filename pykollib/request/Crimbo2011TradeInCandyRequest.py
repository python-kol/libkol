...
class Crimbo2011TradeInCandyRequest(GenericRequest):
    ...
    def parseResponse(self):
        ...
        notEnoughCandyPattern = PatternManager.getOrCompilePattern(
            "crimboNotEnoughCandy"
        )
        if notEnoughCandyPattern.search(self.responseText):
            raise Error.Error(
                "You do not have enough of that candy.", Error.ITEM_NOT_FOUND
            )

        creditsReceivedPattern = PatternManager.getOrCompilePattern(
            "crimboCandyCreditsReceived"
        )
