...
def isOrderedBy(field, response):
    ...
    """
    if field == 'price': previous = -1
    elif field == 'stock': previous = 1000000000
    for shopItem in response['results']:
        if field == 'price':
            if shopItem['price'] < previous: return False
            if shopItem['price'] > previous:
                previous = shopItem['price']
        elif field == 'stock':
            if shopItem['quantity'] > previous: return False
...
def hasLimits(response):
    ...
    """
    for shopItem in response['results']:
        if 'limit' in shopItem and shopItem['limit'] > 0: return True
    return False
...
class Main(unittest.TestCase):
    def assertItemCount(self, response, target, what):
        ...
        """
        self.assertEqual(target, len(response['results']),
                        'Expected {0} {1}, got {2}'
                            .format(target, what, len(response['results'])))
    ...
    def runTest(self):
        ...
        self.assertTrue(isOrderedBy('price', response),
                        'Results for tiny barrel not ordered by ascending price')
        self.assertTrue(hasLimits(response),
                        'Need shops with limits for the next test to work')
        
        # Look up tiny barrel without limits
        misr=MISR(s, 'tiny barrel', noLimits=True)
        response = misr.doRequest()
        self.assertFalse(hasLimits(response),
                        'Search failed to eliminate shops with limits')
        
        # Look up tiny barrel with price limit
        misr=MISR(s, 'tiny barrel', maxPrice=priceLimit)
        response = misr.doRequest()
        self.assertTrue(countBelowGap == len(response['results']),
                        'Expected {0} items below ${1}, found {2}'
                            .format(countBelowGap, priceLimit, 
        ...
        response = misr.doRequest()
        self.assertTrue(isOrderedBy('stock', response),
        ...
        response = misr.doRequest()
        self.assertItemCount(response, 46, 
                             'worthwhile (good, awesome, epic) wines')
