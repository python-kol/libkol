...
def open_test_data(request, variant: str):
    return open(path.join(TEST_DATA, "{}_{}.html".format(request, variant)))

class MockSession:
    def __init__(self, test, request_mocks):
        Response = namedtuple("ClientResponse", ["url", "content", "get_encoding", "text"])
        ...
        def async_return(url, *args, **kwargs):
            ...
            response = Response(url=url, content=content, get_encoding=lambda: "utf-8", text=lambda *args: text)
...
class TestCase(unittest.TestCase):
    ...
    def run_async(self, data, async_test, request_mocks = {}):
        ...
        async def run_test():
            await Tortoise.init(
                ...
                modules={'models': ['pykollib.FoldGroup', 'pykollib.Item', 'pykollib.ZapGroup']}
            )

            Model.kol = session

            try:
                with open_test_data(self.request, data) as file:
